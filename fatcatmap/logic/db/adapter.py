...
class WarehouseAdapter(abstract.GraphModelAdapter):
  ...
  ''' Specifies an abstract adapter that is capable of supporting
      proprietary, ``fatcatmap``-related driver functionality. '''

  class KeyTranslator(object):

    ''' Specifies an attached object to a ``WarehouseAdapter`` that
        knows how to translate various keys to various other kinds
        of keys. '''

    class node(object):

      ''' Specifies key transformations from a ``Node``-centric
          point of view. '''

      @staticmethod
      def to_hint(key, **spec):

        ''' Converts a ``Node`` key to a ``Hint`` key, given a
            ``spec`` where the ``Node``'s ``key`` is taken as
            the origin. '''
      ...
      def to_native(key, type):
        ...
        ''' Converts a ``Node`` key to a ``Native`` key, given
      ...
      def to_descriptor(key, type, name=None):
        ...
        ''' Converts a ``Node`` key to a ``Descriptor`` key,
            given the target ``Descriptor`` ``type`` and an
            optional ``name`` specification. '''
    ...
    class hint(object):
      ...
      ''' Specifies key transformations from a ``Hint``-centric
          point of view. '''
    ...
    class edge(object):
      ...
      ''' Specifies key transformations from a ``Edge``-centric
          point of view. '''
      ...
      def to_native(key, type):
        ...
        ''' Converts an ``Edge`` key to a ``Native`` key, given
            the target ``Native`` type. '''
      ...
      def to_descriptor(key, type, name=None):
        ...
        ''' Converts an ``Edge`` key to a ``Descriptor`` key,
            given the target ``Descriptor`` ``type`` and an
            optional ``name`` specification. '''
  ...
  def _hints(self, subject, **kwargs):
    ...
    ''' Internal method that dispatches matching implementation
        ``hints`` on child classes. '''
  ...
  def hint(self, subject, data=None, **kwargs):  # pragma: no cover
    ...
    ''' Specifies an abstract interface for retrieving or writing
        graph ``Hint`` objects, which accelerate queries by summarizing
        various traversed Graph fragments. '''
  ...
  def attach(self, subject, descriptor, **kwargs):  # pragma: no cover
    ...
    ''' Specifies an abstract interface for writing descriptors,
        which are tiny decorator objects attached to parent objects. '''
  ...
  def native(self, subject, version=None, **kwargs):  # pragma: no cover
    ...
    ''' Specifies an abstract interface for retrieving an object's
        ``Native``, which contains implementation-specific data.

        Optionally, a specific ``Native`` ``version`` can be
        requested. '''
  ...
  def descriptors(self, subject, type=None, **kwargs):  # pragma: no cover
    ...
    ''' Specifies an abstract interface for retrieving an object's
        ``Descriptor``s, which are tiny decorator objects attached
        to parent objects to specify extra data or metadata. '''
...
class DatastoreWarehouse(WarehouseAdapter):
  ...
  def neighbors(self, key, type=None, **kwargs):
    ...
    ''' Retrieve neighbor nodes, which are peers across ``edges``,
        for ``key``, optionally filtered by ``type``, from the Datastore. '''
  ...
  def attach(self, subject, descriptor, **kwargs):
    ...
    ''' Attach a new ``descriptor`` object to a ``subject`` key,
        which contains extra ancillary data, in the Datastore. '''
  ...
  def descriptors(self, subject, type=None, **kwargs):
    ...
    ''' Retrieve attached ``Descriptor`` objects for a given ``subject``
        key, optionally filtered by ``type``, from the Datastore. '''
...
class RedisWarehouse(WarehouseAdapter, redis.RedisAdapter):
  ...
  def neighbors(self, key, type=None, **kwargs):
    ...
    ''' Retrieve neighbor nodes, which are peers across ``edges``,
        for ``key``, optionally filtered by ``type``, from Redis. '''
  ...
  def attach(self, subject, descriptor, **kwargs):
    ...
    ''' Attach a new ``descriptor`` object to a ``subject`` key,
        which contains extra ancillary data, in Redis. '''
