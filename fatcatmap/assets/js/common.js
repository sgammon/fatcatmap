(function() {
  ...
  this.stage = _get('appstage');

  this.map = _get('map');

  this.frame = _get('appframe');

  this.image_prefix = "//fatcatmap.org/image-proxy/providence-clarity/warehouse/raw/govtrack/photos/";

  this.__onload_callbacks = [];
  ...
  load_context = this.load_context = function(event, data) {
    this.context = data || JSON.parse(document.getElementById('js-context').textContent);
    console.log("Loading context...", this.context);
    if (this.context.services) {
      console.log("Loading services...", this.context.services);
      apptools.rpc.service.factory(this.context.services);
    }
    if (this.context.pagedata) {
      this.pagedata = JSON.parse(document.getElementById('js-data').textContent);
      console.log("Detected stapled pagedata...", this.pagedata);
      this.receive(this.pagedata);
    }
    return this.context;
  };

  this.__onload_callbacks.push(load_context);
